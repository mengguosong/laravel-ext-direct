...
class ExtDirect
{
    ...
    static public function get_api_array()
    {
        ...
        foreach( self::$api_classes as $api_name => $class )
        {
            $methods = array();
...
//            if (!!preg_match('/@directname ([\w]+)/', $reflection->getDocComment(), $matches)) {
//                $class = $matches[1];
//            }
...
JAVASCRIPT;
    ...
    {
        self::config(config('Bulforce.ext-direct.config'));

        $c = new ExtDirectController($api_classes, false);
        $c->run();
        $response = $c->getResponseContents();
        return $response;
    }
...
class ExtDirectAction
{
    ...
    protected function call_action()
    {
        ...
        if(array_key_exists($class, ExtDirect::$api_classes)) {
            $class = ExtDirect::$api_classes[$class];
        }
